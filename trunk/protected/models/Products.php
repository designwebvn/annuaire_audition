 ...
 * @property integer $oder_product_id
 ...
 * @property ProductsCenters[] $productsCenters
 * @property ProductsSpecialEducationSchools[] $productsSpecialEducationSchools
 * @property ProductsSpeechTherapists[] $productsSpeechTherapists
 */
class Products extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
            ...
            array('name, alias, type, image, pdf', 'length', 'max'=>255),
            array('created, updated, description, grip', 'safe'),
	...
	public function relations()
	{
		...
		return array(
			...
			'productGalleries' => array(self::HAS_MANY, 'ProductGalleries', 'product_id'),
            'productsCenters' => array(self::HAS_MANY, 'ProductsCenters', 'products_id'),
            'productsSpecialEducationSchools' => array(self::HAS_MANY, 'ProductsSpecialEducationSchools', 'products_id'),
            'productsSpeechTherapists' => array(self::HAS_MANY, 'ProductsSpeechTherapists', 'products_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'oder_product_id' => Yii::t('global', 'Link other articles'),
        ...
        foreach ($blockRents as $blockRent) {
            ...
            $html .= '<li>
						<div class="trans2 slide-sngl">
						   <a class="text-center" href="/articles/view/'.$blockRent['alias'].'">'.$image.'</a>
						   <h4><a href="/articles/view/'.$blockRent['alias'].'">'.$blockRent["name"].'</a></h4>
						   <p>'.$description.'</p>
    ...
    public function renCatproduct($cat_id){
        ...
        if($blockRents){
            $html .='<div class="news-sec-1 float-width">
				 ...
				 <h3 class="sec-title"><a href="articles/category/'.$cat->alias.'">'.$cat->name.'</a></h3>';

            foreach ($blockRents as $blockRent) {
                $image 	 = $this->renImageProduct($blockRent["image"],'','blocky');
                $html .= '<div class="sec-1-sm">
								 <a href="/articles/view/'.$blockRent["alias"].'">'.$image.'</a>
								 <div class="sec-1-sm-text blocky">
									<h3><a href="/articles/view/'.$blockRent["alias"].'">'.$blockRent["name"].'</a></h3>
    ...
    function getProductOthersFE( $id ){
        $catProducts        = ProductOthers::model()->findAllByAttributes(array('product_id' => $id));
        $id                 = array();
        foreach ( $catProducts as $key=>$val ){
            $id[$key] = $val['other_product_id'];
        }
        $ids = implode(',',$id);
        $ids = ( $ids )? $ids :0;
        $listProduct        = Products::model()->findAll('id IN ('.$ids.')');
        $listCatProduct     = "";
        foreach($listProduct as $catProduct){
            $listCatProduct .= " - <a href=".Yii::app()->createUrl('products/view/'.$catProduct['alias']).">".$catProduct['name']."</a><br>";
        }
        return $listCatProduct;
    }
    ...
    public function getOtherProductId(){
        if ($this->id){
            $models = ProductOthers::model()->findAllByAttributes(array('product_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
                $arr[] = $model->other_product_id;
    ...
    function getProductOthers( $id ){
        ...
        $listCatProduct     = "";
        foreach($listProduct as $catProduct){
            $listCatProduct .= "<label class='catProduct'> <a href=".Yii::app()->createUrl('admin/articles/view?id='.$catProduct['id']).">".$catProduct['name']."</a></label>";
        }
        return $listCatProduct;
    }

    function getProductsCenters($id){
        $catProducts = ProductsCenters::model()->findAllByAttributes(array('products_id' => $id));
        $listCatProduct ="";
        foreach($catProducts as $catProduct){
    ...
    function getProductsSpeechTherapists($id){
        $catProducts = ProductsSpeechTherapists::model()->findAllByAttributes(array('products_id' => $id));
        $listCatProduct ="";
        foreach($catProducts as $catProduct){
    ...
    function getProductsSpecialEducationSchools($id){
        $catProducts = ProductsSpecialEducationSchools::model()->findAllByAttributes(array('products_id' => $id));
        $listCatProduct ="";
        foreach($catProducts as $catProduct){
    ...
    public function getProductsCentersId(){
        if ($this->id){
            $models = ProductsCenters::model()->findAllByAttributes(array('products_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
    ...
    public function getProductsSpeechTherapistId(){
        if ($this->id){
            $models = ProductsSpeechTherapists::model()->findAllByAttributes(array('products_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
    ...
    public function getProductsSpecialEducationSchoolsId(){
        if ($this->id){
            $models = ProductsSpecialEducationSchools::model()->findAllByAttributes(array('products_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
