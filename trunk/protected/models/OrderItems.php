...
class OrderItems extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
            ...
            'ProductPackage' => array(self::BELONGS_TO, 'ProductPackage', 'item_id'),
...
//        if ($this->type == self::DIRECT_BUY ){
    ...
    function showItemPackage(){
        $package = ProductPackage::model()->findByPk($this->item_id);
        if($package){
            return '<a class="description_purple" href="/admin/productPackage/view?id='.$package->id.'">'.$package->group->group_name.'</a>';
        }else{
            return '';
        }
    }
    ...
    public function getInfoProductOrder($id,$shop_id){
        ...
        if($orderItem){
            ...
            foreach($orderItem as $order){
                if($shop_id ==0){
                    $result.= "<label class='product_name_order'>".$order->qty."x ".$order->ProductPackage->group['group_name']; //." - ".$order->Products['short_desciption']
                } else {
                    $result.= "<label class='product_name_order'>".$order->qty."x ".$order->ProductPackage->group['group_name'];
                }
