 ...
 * @property EventCenters[] $eventCenters
 * @property EventNews[] $eventNews
 * @property EventSpeechTherapist[] $eventSpeechTherapists
 * @property EventTrainingCenters[] $eventTrainingCenters
 * @property EventVideos[] $eventVideoses
 */
class Events extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
            ...
            'eventCenters' => array(self::HAS_MANY, 'EventCenters', 'event_id'),
            'eventNews' => array(self::HAS_MANY, 'EventNews', 'event_id'),
            'eventSpeechTherapists' => array(self::HAS_MANY, 'EventSpeechTherapist', 'event_id'),
            'eventTrainingCenters' => array(self::HAS_MANY, 'EventTrainingCenters', 'event_id'),
            'eventVideoses' => array(self::HAS_MANY, 'EventVideos', 'event_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'starting_date' => Yii::t('events', 'Starting Date'),
    ...
    public function getEventSpeechTherapistId(){
        if ($this->id){
            $models = EventSpeechTherapist::model()->findAllByAttributes(array('event_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
    ...
    public function getEventTrainingCentersId(){
        if ($this->id){
            $models = EventTrainingCenters::model()->findAllByAttributes(array('event_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
    ...
    public function getEventCentersId(){
        if ($this->id){
            $models = EventCenters::model()->findAllByAttributes(array('event_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
    ...
    public function getEventVideoId(){
        if ($this->id){
            $models = EventVideos::model()->findAllByAttributes(array('event_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
                $arr[] = $model->videos_id;
    ...
    public function getEventNewsId(){
        if ($this->id){
            $models = EventNews::model()->findAllByAttributes(array('event_id' => $this->id));
            $arr = array();
            foreach ($models as $model)
                $arr[] = $model->news_id;
    ...
    function getEventNews( $id ){
        $catEvent = EventNews::model()->findAllByAttributes(array('event_id' => $id));
        $listCatProduct ="";
        foreach($catEvent as $catProduct){
    ...
    function getEventVideos( $id ){
        $catEvent = EventVideos::model()->findAllByAttributes(array('event_id' => $id));
        $listCatProduct ="";
        foreach($catEvent as $catProduct){
            $listCatProduct .= "<label class='catProduct'> <a href=".Yii::app()->createUrl('admin/videos/view/?id='.$catProduct['videos']['id']).">".$catProduct['videos']['title']."</a></label>";
        }
        return $listCatProduct;
    }

    function getEventCenters( $id ){
        $catEvent = EventCenters::model()->findAllByAttributes(array('event_id' => $id));
        $listCatProduct ="";
        foreach($catEvent as $catProduct){
    ...
    function getEventTrainingCenters( $id ){
        $catEvent = EventTrainingCenters::model()->findAllByAttributes(array('event_id' => $id));
        $listCatProduct ="";
        foreach($catEvent as $catProduct){
            $listCatProduct .= "<label class='catProduct'> <a href=".Yii::app()->createUrl('admin/trainingCenters/view/?id='.$catProduct['trainingCenters']['id']).">".$catProduct['trainingCenters']['school_name']."</a></label>";
        }
        return $listCatProduct;
    }

    function getEventSpeechTherapists( $id ){
        $catEvent = EventSpeechTherapist::model()->findAllByAttributes(array('event_id' => $id));
        $listCatProduct ="";
        foreach($catEvent as $catProduct){
