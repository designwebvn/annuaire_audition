...
class BlogController extends SiteBaseController {
    ...
    public function actionIndex() {
        ...
        $posts = new CActiveDataProvider('Blog' ,array(
            'criteria'=>array(
                'condition' => "status = 1 AND language ='".Yii::app()->language."'",
        ...
        if( isset($posts) ){
            ...
            $posts = new CActiveDataProvider('Blog' ,array(
                'criteria'=>array(
                    'condition' => "status = 1 AND language ='".$lang."'",
                    'limit' => self::PAGE_SIZE,
                    'order' => 'id DESC '
                )
            ));
        }
        $this->render('view_category', array( 'posts' => $posts ));
    }
	...
	public function actionviewpost()
	{
		if( isset($_GET['alias']) && ( $model = Blog::model()->with(array('category', 'author', 'comments', 'commentscount'))->find('t.alias=:alias', array(':alias'=> Blog::model()->getAlias( $_GET['alias'] ) )) ) )
		{
		    ...
		    $sql               = "SELECT * FROM blogposts WHERE status = 1 AND language = '".Yii::app()->language."' AND alias !='".$_GET['alias']."' ORDER BY id DESC";
            $related_news      = Yii::app()->db->createCommand( $sql )->queryAll();
            
			$this->render('view_post',array( 'content'=>$content, 'model' => $model, 'related_news' => $related_news ));
		}
		else
		{
			throw new CHttpException(404, Yii::t('error', 'Sorry, We could not find that post.'));
