...
class Categories extends CActiveRecord
{
    ...
    public function getChildCategory( $id ){
        return Categories::model()->findAllByAttributes( array('parent_id' => $id ) );
    }
    
    public function getParentCategory( $alias ){
        return Categories::model()->find( 'alias =:alias', array( ':alias'=> $alias ) );
    }
    ...
    public function checkCategory($id){
        $checkCat = ProductCategories::model()->countByAttributes(array('category_id'=>$id));
        if($checkCat>0){
            return false;
        }else  {
            return true;
	...
	public function getNameCat($cate_id){
		$result = Categories::model()->find(array(
					'select'=>'name',
					'condition'=>'id=:id',
	...
	public function getCatById($cate_id){
		$result = Categories::model()->find(array(
					'condition'=>'id=:id',
					'params'=>array( ':id'=>$cate_id ) )
				);
	...
	public function getCatByAlias($alias){
        return Categories::model()->findByAttributes(array('alias'=>$alias));
    }
}